...
/* Raw data buffers — mirrors  short gyro[3], accel[3]  globals */
static short  accel[3];
static short  gyro[3];
static short  temperature_raw;

/*
 * BMI160_vInit — mirrors IMU9x_vInit()
 */
...
//	ts_Bmi160_Interrupt *pInt;
	...
	/* Mirror: sensor data refreshed at the top of every Ioctl call */
	BMI160_ReadSensors();
	...
	switch (eCommand)
	{
		...
		/* -- Accelerometer raw -- */
		case E_BMI160_IOCTL_GET_ACC_RAW_X: *(int16_t *)vpParam = accel[0]; break;
		case E_BMI160_IOCTL_GET_ACC_RAW_Y: *(int16_t *)vpParam = accel[1]; break;
		case E_BMI160_IOCTL_GET_ACC_RAW_Z: *(int16_t *)vpParam = accel[2]; break;

		/* -- Accelerometer scaled (mirrors /16384.0 from MPU9150) -- */
		case E_BMI160_IOCTL_GET_ACC_SCALED_X: *(float *)vpParam = (float)accel[0] * BMI160_ACC_SCALE_4G; break;
		case E_BMI160_IOCTL_GET_ACC_SCALED_Y: *(float *)vpParam = (float)accel[1] * BMI160_ACC_SCALE_4G; break;
		case E_BMI160_IOCTL_GET_ACC_SCALED_Z: *(float *)vpParam = (float)accel[2] * BMI160_ACC_SCALE_4G; break;

		/* -- Gyroscope raw -- */
		case E_BMI160_IOCTL_GET_GYRO_RAW_X: *(int16_t *)vpParam = gyro[0]; break;
		case E_BMI160_IOCTL_GET_GYRO_RAW_Y: *(int16_t *)vpParam = gyro[1]; break;
		case E_BMI160_IOCTL_GET_GYRO_RAW_Z: *(int16_t *)vpParam = gyro[2]; break;

		/* -- Gyroscope scaled (mirrors /131.072 from MPU9150) -- */
		case E_BMI160_IOCTL_GET_GYRO_SCALED_X: *(float *)vpParam = (float)gyro[0] * BMI160_GYR_SCALE_500DPS; break;
		case E_BMI160_IOCTL_GET_GYRO_SCALED_Y: *(float *)vpParam = (float)gyro[1] * BMI160_GYR_SCALE_500DPS; break;
		case E_BMI160_IOCTL_GET_GYRO_SCALED_Z: *(float *)vpParam = (float)gyro[2] * BMI160_GYR_SCALE_500DPS; break;
		...
		case E_BMI160_IOCTL_GET_ALL_DATA:
			...
			pData->rawAccX = accel[0]; pData->rawAccY = accel[1]; pData->rawAccZ = accel[2];
			pData->rawGyrX = gyro[0];  pData->rawGyrY = gyro[1];  pData->rawGyrZ = gyro[2];
