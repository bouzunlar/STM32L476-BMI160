...
#define DETECTION_TASK_PRIORITY     1u
...
//		Debug_Print("[TASK] Sensor running\r\n");
		...
		if (Bmi160_Ioctl(E_BMI160_IOCTL_GET_ALL_DATA, &localData) == E_BMI160_ERR_NONE)
		{
			...
			{
				Debug_Print(" vSensorReadTask xSensorQueue is full!\r\n");
			}
...
void vDataProcessingTask(void *argument)
{
	...
	for (;;)
	{
//		Debug_Print("[TASK] Processing running\r\n");

		ts_Bmi160_Data rawData;

		if (xQueueReceive(xSensorQueue, &rawData, portMAX_DELAY) == pdTRUE)
		{

//			Debug_Print("[PROC] Data received\r\n");
//TODO: data proccessing algorithm need to be implemented.

			if (xQueueSend(xDetectionQueue, &rawData,
					pdMS_TO_TICKS(5)) != pdTRUE)
			{
				Debug_Print("xDetectionQueue is full!\r\n");
			}
		}
		else
		{
			Debug_Print("Cant receive xSensorQueue !\r\n");
		}
	}
}
	...
	for (;;)
	{
		...
		if (windowFull)
		{
			...
			if (xSemaphoreTake(xUartMutex, pdMS_TO_TICKS(10)) == pdTRUE)
			{
				...
				{
					printf("\r\n************************************************\r\n");
					printf("[ALARM] CIRCULAR MOTION DETECTED!\r\n");
					printf("************************************************\r\n");
				}
...
void Debug_Print(const char *msg)
{
	if (xSemaphoreTake(xUartMutex, pdMS_TO_TICKS(20)) == pdTRUE)
	{
		printf("%s", msg);
		xSemaphoreGive(xUartMutex);
	}
